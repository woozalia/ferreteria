...
trait ftPageMenu {
    ...
    */
	...
	if (is_null($this->oMNode)) {
	    ...
	    $oArgs = $this->PathArgObject();
	    if ($oArgs->KeyExists('page')) {	// if a page is specified
		$sPage = $oArgs->GetString('page');
		if (is_array($sPage)) {
	...
	Also, this whole subsystem is a wreck and a tangle.
    */
    private $oMenuMap;
    ...
    */
	...
	if (empty($this->oMenuRoot)) {
	    ...
	    $this->oMenuRoot = new fcMenuRoot($this->MenuManager());
