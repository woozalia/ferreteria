...
abstract class fcRecord_keyed_single extends fcRecord_keyed {
	...
	$sKey = $this->GetTableWrapper()->KeyName();
	if ($this->FieldIsSet($sKey)) {
	    $out = $this->GetFieldValue($sKey);
	} else {
	    $out = NULL;
	}
	return $out;
	...
	return $this->GetTableWrapper()->KeyName().'='.$this->GetKeyValue_cooked();
    }
	...
	return $this->FetchRows_asArray($this->GetTableWrapper()->KeyName());
    }
